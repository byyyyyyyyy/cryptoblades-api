...
exports.task = async () => {
  ...
  for(var chain in chains) {
    ...
    console.log(
      '[' + chains[chain] + '-' + chainName + ':Clean-Up]'
    );
    ...
    const checkFinalPrice = (address, items) => {
      queue.add(async () => {
        ...
        const multicallData = marketplaceHelper.getFinalPriceCall(chainHelper.getMarketAddress(chains[chain]), items);

        const prices = await pRetry(() => multicall(chainHelper.getWeb3(chains[chain]), chainHelper.getMulticallAddress(chains[chain]), multicallData.abi, multicallData.calls), { retries: 5 });
