...
const chainIteration = async (chain) => {
  ...
  const runQueue = async (address, idKey, page) => {
    ...
    results.forEach((item) => {
      reviewedIds[address] += 1;
      queue.add(async () => {
        const price = +(await marketplaceHelper.getNftMarketPlaceForSync(chain, chainHelper.getMarketAddress(chain), chainHelper.getRPC(chain)).methods.getFinalPrice(address, item[idKey]).call());
